...
Else 
	...
	If (This:C1470=Null:C1517)
		...
		$agent:=cs:C1710._AgentRemoteTexts.new("Azure_xAI"; "gpt-5.4-1")
		//$agent:=cs._AgentRemoteTexts.new("Azure_xAI"; "grok-4-20-reasoning")
		...
		If ($results#Null:C1517)
			For each ($result; $results)
				For each ($texts; $result.texts)
					...
					While ($target_folder.file("contract-"+$texts.language+"-"+$fileName+".txt").exists)
						$i+=1
						$fileName:=String:C10($i; "000000")
					End while 
					$target_folder.file("contract-"+$texts.language+"-"+$fileName+".txt").setText($texts.text)
				End for each 
			End for each 
		End if 
